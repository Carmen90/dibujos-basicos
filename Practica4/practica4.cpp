...
*/
void datosIdentificacion ( string& nombre);
...
void añadirProducto ( tLista& lista){
	...
	buscarProducto ( lista, clave, encontrado, posicion);
	...
	if ( correcto){
		...
		if (!encontrado)
			...
			if ( c == 's' || c == 'S'){
				...
				lista.elementos[posicion].identificacion = nombre;
			}
